...
describe('BillingService', () => {
    ...
    describe('generateMonthlyBilling', () => {
        it('should generate invoices for active subscriptions with correct VAT', async () => {
            const subs = [
                { id: 's1', userId: 'u1', monthlyAmount: 100, status: 'ACTIVE', startDate: '2023-01-01' },
            ];
        ...
        it('should apply B1M20 promo code (50% off first month)', async () => {
            const subs = [
                { id: 's1', userId: 'u1', monthlyAmount: 100, status: 'ACTIVE', promoCode: 'B1M20' },
            ];
            vi.mocked(subscriptionRepo.findAll).mockResolvedValue(subs as any);
            // First invoice
    ...
    describe('updatePaymentStatuses', () => {
        it('should update invoice status to PAID if executed', async () => {
            ...
            await service.updatePaymentStatuses(
                [{ invoiceId: 'i1', status: 'EXECUTED' }]
            );
